...
class CommentList extends React.Component 
{
    ...
    {
        console.log("dcscscd")
        if(!this.props.Open&&Open&&!article.commentsLoaded&&!article.commentsLoading) {loadComments(article.id)};
    }
    render(){
        return (
            <div>
                <button onClick={this.props.showToggle}>{this.props.Open ? "Hide comments" : "Show comments"} </button>
